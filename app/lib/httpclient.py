    ...
    def request(self, method, url, data=None, json=None, file=None, custom=None, saveToFile=None, headers={}):
        ...
        if redirect:
            ...
            if status in [301, 302, 303]:
                return self.request('GET', url=redirect, data=data, json=json, file=file, custom=custom,
                                    saveToFile=saveToFile, headers=headers)
            else:
                return self.request(method, redirect, data=data, json=json, file=file, custom=custom,
                                    saveToFile=saveToFile, headers=headers)
        else:
